...
async function fetchProps() {
  ...
  const [laRes, ieRes] = await Promise.all([
    fetch('https://rently.purpletree-c228d976.eastus.azurecontainerapps.io/enweb/los%20angeles'),
    fetch('https://rently.purpletree-c228d976.eastus.azurecontainerapps.io/enweb/inland%20empire')
  ]);
  ...
  // Set label to combined LA/IE since we're always showing both
  const label = 'Los Angeles & Inland Empire';
  document.getElementById('page-title').textContent = `${label} Properties`;

  initFilters();
  applyFilters();

  // populate market dropdown - simplified since we're only showing combined view
  const marketSelect = document.getElementById('market-select');
  marketSelect.innerHTML = `
    <option value="">All Markets</option>
    <option value="los angeles,inland empire" selected>Los Angeles & Inland Empire</option>
  `;
...
function match(p) {
  // No market filter since we're showing combined LA/IE
  ...
  const baf = +getVal('filter-bathrooms');
  if (baf && p.bathrooms < baf) return false;
  
  const min = +document.getElementById('price-min').value;
  if (p.rent < min) return false;
...
function applyFilters() {
  ...
  if (rentalLink) {
    ...
    url.searchParams.set('market', 'los angeles,inland empire');
...
function render() {
  ...
  pageItems.forEach(p => {
    ...
    card.innerHTML = `
      <div style="position:relative">
        ...
        <div class="availability-banner"
             style="background:${bnr === 'Coming Soon' ? '#6300FF' : '#1BDBEA'}">
          ${bnr}
        </div>
      </div>
    ...
    // Use generic URL for combined view
    const detailPageUrl = `/long-beach-homes-for-rent/listings-detail?id=${p.tag}`;
    
    card.onclick = () => location.href = detailPageUrl;
    card.addEventListener('mouseenter', () => {
      const info = markerMap[p.tag];
      if (info) {
        map.jumpTo({ center: info.lngLat, zoom: 13 });
        popup.setLngLat(info.lngLat).setHTML(`
          <a href="${detailPageUrl}" style="display:block; text-decoration:none; color:inherit;">
            <div style="
            ...
            ">
              ...
              <div style="margin-top:8px;">
                ...
                &nbsp;|&nbsp; <a href="${detailPageUrl}" style="color:#1ADCEA; text-decoration:underline;">View</a>
              </div>
            </div>
          </a>
        `).addTo(map);
    ...
    fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(p.city + ', ' + p.state + ' ' + p.zip)}.json?access_token=${mapboxgl.accessToken}`)
      ...
      .then(js => {
        ...
        const popupHTML = `
          <a href="${detailPageUrl}" style="display:block; text-decoration:none; color:inherit;">
            <div style="
              ...
              width: 260px;
            ">
              <img src="${(p.images?.[0] || 'img/placeholder.png')}"
                   style="width:100%; height:140px; object-fit:cover; border-radius:4px;" />
              <div style="margin-top:8px;">
        ...
        el.innerHTML = `<div style="background:#1ADCEA;color:#fff;
                         border-radius:4px;padding:4px;
                         font-size:0.8rem;font-family:'Arvo',serif;
                         cursor:pointer;">
                       $${p.rent}
                       </div>`;
