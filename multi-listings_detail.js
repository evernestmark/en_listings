mapboxgl.accessToken = 'pk.eyJ1Ijoic3JvYmVza3ktZXZlcm5lc3QiLCJhIjoiY204aHNlb3loMDNyYTJtb2d3MGs3d25oMSJ9.lUlpnvq3X1UciyTPie6X_Q';    let allPhotos = [], currentPhotoIndex = 0, videoUrl;
  ...
  async function fetchPropertyDetails() {
    ...
    // Fetch from both LA and IE endpoints
    ...
    const laList = await laRes.json();
    const ieList = await ieRes.json();
    
    // Combine the lists and find the property
    const combinedList = [...laList, ...ieList];
    const property = combinedList.find(p => String(p.tag) === id);
    ...
    // Related Properties - search within the combined list
    const related = combinedList.filter(p =>
    ...
    if (related.length) {
      related.forEach(p => {
        ...
        div.innerHTML = `
          ...
          `;
        div.onclick = () => location.href = "/long-beach-homes-for-rent/listings-detail?id=" + p.tag;
    ...
    backBtn.href = `/long-beach-homes-for-rent/listings?market=${property.market_id.toLowerCase()}`;
    const rentalLink = document.getElementById('rental-quals-link');
    if (rentalLink) {
      rentalLink.href = `/long-beach-homes-for-rent/rental-qualifications?state=${encodeURIComponent(property.state)}`;
    }
  ...
  function initMap(addr) {
    const m = new mapboxgl.Map({container:"map",style:"mapbox://styles/mapbox/streets-v11",center:[-118.2437, 34.0522],zoom:10});
